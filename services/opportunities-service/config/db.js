...
const connectDB = async (attempt = 1) => {
  try {
    ...
    console.log('[opportunities-service] MongoDB connected');
  } catch (err) {
    console.error(`[opportunities-service] MongoDB connection failed (attempt ${attempt}/${MAX_RETRIES}):`, err.message);
    if (attempt >= MAX_RETRIES) {
      console.error('[opportunities-service] Giving up after max retries - exiting.');
      process.exit(1);
    }
