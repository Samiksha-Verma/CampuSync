...
const connectDB = async (attempt = 1) => {
  try {
    ...
    console.log('[certification-service] MongoDB connected');
  } catch (err) {
    console.error(`[certification-service] MongoDB connection failed (attempt ${attempt}/${MAX_RETRIES}):`, err.message);
    if (attempt >= MAX_RETRIES) {
      console.error('[certification-service] Giving up after max retries - exiting.');
      process.exit(1);
    }
