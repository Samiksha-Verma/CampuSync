...
const connectDB = async (attempt = 1) => {
  try {
    ...
    console.log('[events-service] MongoDB connected');
  } catch (err) {
    console.error(`[events-service] MongoDB connection failed (attempt ${attempt}/${MAX_RETRIES}):`, err.message);
    if (attempt >= MAX_RETRIES) {
      console.error('[events-service] Giving up after max retries - exiting.');
      process.exit(1);
    }
