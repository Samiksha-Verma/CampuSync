...
const connectDB = async (attempt = 1) => {
  try {
    ...
    console.error(`[auth-service] MongoDB connection failed (attempt ${attempt}/${MAX_RETRIES}):`, err.message);
    if (attempt >= MAX_RETRIES) {
      console.error('[auth-service] Giving up after max retries - exiting.');
      process.exit(1);
    }
