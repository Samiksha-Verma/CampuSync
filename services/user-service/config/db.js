...
const connectDB = async (attempt = 1) => {
  try {
    ...
    console.error(`[user-service] MongoDB connection failed (attempt ${attempt}/${MAX_RETRIES}):`, err.message);
    if (attempt >= MAX_RETRIES) {
      console.error('[user-service] Giving up after max retries - exiting.');
      process.exit(1);
    }
