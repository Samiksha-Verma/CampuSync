...
const connectDB = async (attempt = 1) => {
  try {
    ...
    console.log('[document-vault-service] MongoDB connected');
  } catch (err) {
    console.error(`[document-vault-service] MongoDB connection failed (attempt ${attempt}/${MAX_RETRIES}):`, err.message);
    if (attempt >= MAX_RETRIES) {
      console.error('[document-vault-service] Giving up after max retries - exiting.');
      process.exit(1);
    }
